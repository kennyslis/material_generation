...
class AdvancedTwoDMaterialScreener:
    ...
    def __init__(self, input_dir: str, output_dir: str,
                 ...
                 weight_config: Dict = None, tsa_weight_config: Dict = None):
        ...
        self.tsa_weight_config = tsa_weight_config
    ...
    def _integrate_tsa_results(self, topo_features: Dict, graph_features: Dict,
        ...
        TSA内部权重配置（默认，可通过tsa_weight_config自定义）：
        - 拓扑特征 (持续同调): 40%
        - 图论特征 (平面性): 30%
        - 几何特征 (PCA): 30%

        注: 可通过tsa_weight_config参数自定义权重配置，用于消融实验
        """
        dimension_scores = {'0D': 0.0, '1D': 0.0, '2D': 0.0, '3D': 0.0}

        # 允许自定义TSA内部权重配置（用于消融实验）
        tsa_weight_config = getattr(self, 'tsa_weight_config', {
            'topology': 0.40,
            'graph': 0.30,
            'geometric': 0.30
        })
        ...
        if 'h1_features' in topo_features:
            ...
            dimension_scores['2D'] += tsa_weight_config['topology'] * min(1.0, h1_persistence / 10.0)

        # 图论特征权重 (默认30%)
        if 'planarity' in graph_features:
            planarity_score = graph_features['planarity'].get('planar_score', 0)
            dimension_scores['2D'] += tsa_weight_config['graph'] * planarity_score
        ...
        if 'pca_analysis' in geometric_features:
            ...
            for dim, score in pca_scores.items():
                if dim in dimension_scores:
                    dimension_scores[dim] += tsa_weight_config['geometric'] * score
    ...
    def _calculate_vdw_interaction(self, element1: Element, element2: Element, distance: float) -> float:
        """
        计算vdW相互作用 (Lennard-Jones 12-6势)

        参数来源:
        1. UFF力场 (主要来源，覆盖108种元素):
           Rappé, A. K., et al. "UFF, a full periodic table force field for molecular
           mechanics and molecular dynamics simulations."
           J. Am. Chem. Soc. 1992, 114, 10024-10035. (DOI: 10.1021/ja00051a040)

        2. TMD二维材料vdW验证研究:
           Sevik, C., et al. "Role of van der Waals interactions on the binding
           energies of 2D transition-metal dichalcogenides."
           Appl. Surf. Sci. 2023, 609, 155256. (DFT-D3方法验证)

        3. 过渡金属单原子催化剂vdW研究:
           Qiao, L., et al. "Transition metal (Ti, Cu, Zn, Pt) single-atom modified
           graphene/AS2 (A = Mo, W) van der Waals heterostructures."
           Phys. Chem. Chem. Phys. 2023, 25, 23307-23318.

        注意:
        - epsilon单位为eV (已从UFF原始单位K转换: ε(eV) = ε(K) × kB)
        - sigma单位为Å
        - 使用Lorentz-Berthelot组合规则计算不同原子间参数
        """
        vdw_params = {
            # 主族元素
            ...
            'P': {'epsilon': 0.003, 'sigma': 3.7},
            'B': {'epsilon': 0.002, 'sigma': 3.6},
            'Si': {'epsilon': 0.003, 'sigma': 3.8},
            # 过渡金属（常见二维材料）
            'Mo': {'epsilon': 0.004, 'sigma': 3.0},
            'W': {'epsilon': 0.004, 'sigma': 3.1},
            'Ti': {'epsilon': 0.003, 'sigma': 2.9},
            'V': {'epsilon': 0.003, 'sigma': 2.8},
            'Cr': {'epsilon': 0.003, 'sigma': 2.8},
            'Mn': {'epsilon': 0.003, 'sigma': 2.8},
            'Fe': {'epsilon': 0.003, 'sigma': 2.7},
            'Co': {'epsilon': 0.003, 'sigma': 2.7},
            'Ni': {'epsilon': 0.003, 'sigma': 2.7},
            'Cu': {'epsilon': 0.003, 'sigma': 2.8},
            'Zn': {'epsilon': 0.003, 'sigma': 2.8},
            # 硫族元素（TMDs）
            'Se': {'epsilon': 0.004, 'sigma': 3.8},
            'Te': {'epsilon': 0.004, 'sigma': 4.0},
            # 卤素
            'F': {'epsilon': 0.001, 'sigma': 3.0},
            'Cl': {'epsilon': 0.002, 'sigma': 3.4},
            'Br': {'epsilon': 0.003, 'sigma': 3.5},
            'I': {'epsilon': 0.003, 'sigma': 3.7},
        }
    ...
    def _calculate_2d_score_advanced(self, tsa_result: Dict, binding_energy: float, vdw_analysis: Dict) -> float:
        ...
        权重配置（已调整，降低vdW权重）：
        - TSA分析: 55% (提高，因为是最可靠的维度判定方法)
        - 结合能: 35% (提高，DFT/经验计算相对可靠)
        - vdW相互作用: 10% (降低，因为大部分元素使用默认参数，准确性有限)

        注: 可通过weight_config参数自定义权重配置
        """
        score = 0.0

        # 允许自定义权重配置（用于消融实验）
        weight_config = getattr(self, 'weight_config', {
            'tsa': 0.55,
            'binding_energy': 0.35,
            'vdw': 0.10
        })
...
class AblationStudyRunner:
    ...
    def __init__(self, input_dir: str, output_dir: str, sample_size: int = 50):
        ...
        self.test_configs = {
            'Config_A_Topology': {
                'name': '配置A: 仅拓扑特征',
                'tsa_weights': {'topology': 1.0, 'graph': 0.0, 'geometric': 0.0}
            },
            'Config_B_Graph': {
                'name': '配置B: 仅图论特征',
                'tsa_weights': {'topology': 0.0, 'graph': 1.0, 'geometric': 0.0}
            },
            'Config_C_Geometric': {
                'name': '配置C: 仅几何特征',
                'tsa_weights': {'topology': 0.0, 'graph': 0.0, 'geometric': 1.0}
            },
            'Config_D_Default': {
                'name': '配置D: 默认权重(40/30/30)',
                'tsa_weights': {'topology': 0.40, 'graph': 0.30, 'geometric': 0.30}
            }
        }
    ...
    def run_ablation_study(self):
        """运行消融实验"""
        print(f"\n{'='*60}")
        print("开始消融实验 - TSA权重验证")
        print(f"{'='*60}")
        print(f"样本数量: {self.sample_size}")

        # 选择样本
        all_files = list(self.input_dir.glob("*.cif"))
        ...
        print(f"从{len(all_files)}个文件中选择{len(sample_files)}个样本")

        # 保存样本列表
        with open(self.output_dir / "sample_list.txt", 'w') as f:
            for file in sample_files:
                f.write(f"{file.name}\n")

        # 运行所有配置
        all_results = {}
        for config_name, config_info in self.test_configs.items():
            print(f"\n运行: {config_info['name']}")

            # 创建输出目录
            config_output = self.output_dir / config_name
            ...
            screener = AdvancedTwoDMaterialScreener(
                ...
                use_dft=False,
                use_topology=True,
                tsa_weight_config=config_info['tsa_weights']
            )

            # 处理样本
            results = []
            for i, cif_file in enumerate(sample_files):
                if (i + 1) % 10 == 0:
                    print(f"  处理进度: {i+1}/{len(sample_files)}")
            ...
            all_results[config_name] = {
                ...
                'weights': config_info['tsa_weights'],
                'total': len(results),
                'candidates': len(candidates),
                'rate': len(candidates) / len(results) if results else 0,
                'candidate_files': [r['filename'] for r in candidates]
            }

            print(f"  识别为2D: {len(candidates)}/{len(results)} ({all_results[config_name]['rate']:.1%})")

        # 生成对比报告
        self._generate_comparison_report(all_results)

        # 生成人工评估模板
        self._generate_evaluation_template(all_results)
    ...
    def _generate_comparison_report(self, all_results: Dict):
        ...
        for config_name, summary in all_results.items():
            comparison_data.append({
                ...
                '识别为2D数量': summary['candidates'],
                '总样本数': summary['total'],
                '2D比例': f"{summary['rate']:.1%}"
            })

        df = pd.DataFrame(comparison_data)
        excel_file = self.output_dir / "配置对比表.xlsx"
        df.to_excel(excel_file, index=False)
        print(f"\n✓ 配置对比表: {excel_file}")

    def _generate_evaluation_template(self, all_results: Dict):
        """生成人工评估模板"""
        evaluation_data = []

        for config_name, summary in all_results.items():
            for filename in summary['candidate_files']:
                evaluation_data.append({
                    '配置名称': summary['config'],
                    '文件名': filename,
                    '是否为真实2D材料': '',  # 待填写: yes/no
                    '置信度': '',  # 待填写: high/medium/low
                    '备注': ''
                })

        df = pd.DataFrame(evaluation_data)
        excel_file = self.output_dir / "人工评估模板.xlsx"
        df.to_excel(excel_file, index=False)

        print(f"✓ 人工评估模板: {excel_file}")
        print(f"  共{len(evaluation_data)}个候选材料待评估")
        print(f"\n请在Excel中填写:")
        print(f"  - '是否为真实2D材料': 填写 yes 或 no")
        print(f"  - '置信度': 填写 high/medium/low")
        print(f"  - '备注': 可选填写")

    @staticmethod
    def calculate_accuracy(evaluation_file: str, output_dir: str):
        """从人工评估结果计算准确率"""
        print(f"\n{'='*60}")
        print("计算各配置准确率")
        print(f"{'='*60}")
        ...
        # 按配置分组统计
        accuracy_results = []
        for config in df['配置名称'].unique():
            config_df = df[df['配置名称'] == config]

            total = len(config_df)
            true_2d = len(config_df[config_df['是否为真实2D材料'].str.lower() == 'yes'])
            accuracy = true_2d / total if total > 0 else 0

            # 高置信度准确率
            high_conf = config_df[config_df['置信度'].str.lower() == 'high']
            high_acc = len(high_conf[high_conf['是否为真实2D材料'].str.lower() == 'yes']) / len(high_conf) if len(high_conf) > 0 else 0
        ...
        output_file = Path(output_dir) / "准确率分析.xlsx"
        df_acc.to_excel(output_file, index=False)

        print(f"✓ 准确率分析: {output_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='二维材料高精度筛选')
    parser.add_argument('--mode', choices=['screen', 'ablation', 'accuracy'],
                        default='screen', help='运行模式')
    parser.add_argument('--input', default='data/filtered', help='输入目录')
    parser.add_argument('--output', default='data/2d_materials_advanced', help='输出目录')
    parser.add_argument('--sample-size', type=int, default=50, help='消融实验样本数')
    parser.add_argument('--eval-file', help='人工评估文件路径（用于计算准确率）')
    ...
    if args.mode == 'screen':
        ...
        # 消融实验模式
        print("运行模式: 消融实验")
        ablation_output = Path(args.output).parent / "ablation_study"
        runner = AblationStudyRunner(args.input, str(ablation_output), args.sample_size)
        ...
        if not args.eval_file:
            print("错误: 请使用 --eval-file 指定人工评估文件")
            return
        ablation_output = Path(args.output).parent / "ablation_study"
        AblationStudyRunner.calculate_accuracy(args.eval_file, str(ablation_output))
